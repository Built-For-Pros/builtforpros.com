  ...
  return (
    ...
    >
      <div className="content-shell">
        ...
        <div className="mx-auto mt-12 sm:mt-16 grid max-w-3xl gap-4">
          {faqItems.map((item, index) => (
            <ScrollReveal key={item.question} delay={index * 80}>
              <details className="group rounded-xl border border-[color:var(--border-subtle)] bg-[color:var(--elevated-deep)] transition hover:border-[color:var(--border-hover)] open:border-[color:var(--border-hover)] open:bg-[color:var(--elevated)]">
                <summary className="flex cursor-pointer list-none items-start justify-between gap-4 px-6 py-6 text-lg leading-7 font-semibold text-[color:var(--fg)] [&::-webkit-details-marker]:hidden">
                ...
                <p className="border-t border-[color:var(--border-subtle)] px-6 pb-6 pt-4 text-base leading-7 text-[color:var(--muted)]">
                  {item.answer}
                </p>
              </details>
            </ScrollReveal>
